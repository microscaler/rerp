    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("status") {
            data_map.insert(
                "status".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,Some(&serde_json::json!({"enum":["DRAFT","VALIDATING","READY","SUBMITTED","ACCEPTED","REJECTED","RETRY_REQUIRED","CANCELLED"],"type":"string"})),None,None,
                ),
            );
        } else {
...
pub fn handler(req: TypedHandlerRequest<Request>) -> Response {
    crate::controllers::list_edi_submissions::handle(req)
}
