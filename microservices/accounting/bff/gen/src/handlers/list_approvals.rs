...
use crate::handlers::types::AccountsPayableApprovalRecord;
...
impl TryFrom<HandlerRequest> for Request {
    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("invoice_id") {
            data_map.insert(
        ...
        if let Some(v) = req.get_query_param("approver_id") {
            data_map.insert(
        ...
        if let Some(v) = req.get_query_param("action") {
            data_map.insert(
                "action".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,Some(&serde_json::json!({"enum":["APPROVE","REJECT","REQUEST_INFO","DELEGATE"],"type":"string"})),None,None,
                ),
            );
        } else {
