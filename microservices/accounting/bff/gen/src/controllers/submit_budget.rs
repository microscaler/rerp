// User-owned controller for handler 'submit_budget'.
...
pub fn handle(req: HandlerRequest) -> HandlerResponse {
    ...
    let path_str = "/api/budget/budgets/{id}/submit";
    ...
    let service_name_upper = "budget".to_uppercase();
    ...
    let service_fqdn = format!("{}.{}", "budget", domain_suffix);
