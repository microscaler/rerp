...
impl TryFrom<HandlerRequest> for Request {
    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("code") {
            data_map.insert(
                "code".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,
        ...
        if let Some(v) = req.get_query_param("country_code") {
            data_map.insert(
                "country_code".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,
                    Some(&serde_json::json!({"maxLength":2,"type":"string"})),
                    None,
                    None,
                ),
            );
        ...
        if let Some(v) = req.get_query_param("type") {
            data_map.insert(
                ...
                brrtrouter::server::request::decode_param_value(
                    v,
                    Some(&serde_json::json!({"enum":["PERCENTAGE","FIXED"],"type":"string"})),
                    None,
                    None,
                ),
            );
...
pub fn handler(req: TypedHandlerRequest<Request>) -> HttpJson<Response> {
    crate::controllers::list_tax_rates::handle(req)
}
