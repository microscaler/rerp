...
impl TryFrom<HandlerRequest> for Request {
    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("forecast_type") {
            data_map.insert(
                "forecast_type".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,Some(&serde_json::json!({"enum":["ROLLING","ANNUAL","QUARTERLY","SPECIFIC"],"type":"string"})),None,None,
                ),
            );
        } else {
        ...
        if let Some(v) = req.get_query_param("status") {
            data_map.insert(
                "status".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,Some(&serde_json::json!({"enum":["DRAFT","PENDING_REVIEW","APPROVED","ARCHIVED"],"type":"string"})),None,None,
                ),
            );
        } else {
...
pub fn handler(req: TypedHandlerRequest<Request>) -> HttpJson<Response> {
    crate::controllers::list_forecasts::handle(req)
}
