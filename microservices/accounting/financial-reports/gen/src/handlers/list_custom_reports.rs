    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("type") {
            data_map.insert(
                "type".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,Some(&serde_json::json!({"enum":["BALANCE_SHEET","INCOME_STATEMENT","CASH_FLOW","GENERAL_LEDGER","CUSTOM_SQL","AGGREGATION"],"type":"string"})),None,None,
                ),
            );
        } else {
        ...
        if let Some(v) = req.get_query_param("is_shared") {
            data_map.insert(
                "is_shared".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,
...
pub fn handler(req: TypedHandlerRequest<Request>) -> Response {
    crate::controllers::list_custom_reports::handle(req)
}
