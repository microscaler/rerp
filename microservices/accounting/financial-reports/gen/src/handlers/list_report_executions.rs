...
impl TryFrom<HandlerRequest> for Request {
    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("report_id") {
            data_map.insert(
                "report_id".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,
        ...
        if let Some(v) = req.get_query_param("status") {
            data_map.insert(
                "status".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,Some(&serde_json::json!({"enum":["PENDING","RUNNING","COMPLETED","FAILED"],"type":"string"})),None,None,
                ),
            );
        } else {
...
pub fn handler(req: TypedHandlerRequest<Request>) -> Response {
    crate::controllers::list_report_executions::handle(req)
}
