...
pub struct Request {
    ...
    pub repartition_type: Option<String>,
}
...
impl TryFrom<HandlerRequest> for Request {
    ...
    fn try_from(req: HandlerRequest) -> Result<Self, Self::Error> {
        ...
        if let Some(v) = req.get_query_param("tax_id") {
            data_map.insert(
                "tax_id".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,
        ...
        if let Some(v) = req.get_query_param("repartition_type") {
            data_map.insert(
                "repartition_type".to_string(),
                brrtrouter::server::request::decode_param_value(
                    v,
                    Some(&serde_json::json!({"enum":["TAX","BASE"],"type":"string"})),
                    None,
                    None,
                ),
            );
...
pub fn handler(req: TypedHandlerRequest<Request>) -> HttpJson<Response> {
    crate::controllers::list_tax_repartition_lines::handle(req)
}
