...
class TestIsSnakeCase:
    def test_empty_false(self):
        from rerp_tooling.openapi.fix_operation_id import is_snake_case
...
class TestToSnakeCase:
    def test_camel_case(self):
        from rerp_tooling.openapi.fix_operation_id import to_snake_case
...
class TestFindOpenapiFiles:
    def test_finds_yaml_and_yml(self, tmp_path: Path):
        from rerp_tooling.openapi.fix_operation_id import find_openapi_files
...
class TestProcessFile:
    def test_converts_camel_and_writes(self, tmp_path: Path):
        from rerp_tooling.openapi.fix_operation_id import process_file
...
class TestRun:
    def test_missing_dir_returns_0_0(self, tmp_path: Path):
        from rerp_tooling.openapi.fix_operation_id import run

        d = tmp_path / "missing"
        assert not d.exists()
        t, u = run(d, dry_run=False, verbose=False)
        assert t == 0 and u == 0
    ...
    def test_no_openapi_files_returns_0_0(self, tmp_path: Path):
        ...
        t, u = run(tmp_path, dry_run=False, verbose=False)
        assert t == 0 and u == 0
    ...
    def test_one_file_with_changes_returns_total_and_touched(self, tmp_path: Path):
        ...
        t, u = run(tmp_path, dry_run=False, verbose=False, rel_to=tmp_path)
        assert t == 2 and u == 1
