...
- **`rerp docker build-image-simple <image_name> <dockerfile> <hash_path> <artifact_path> [--service NAME]`** — When `--service` is set, `dockerfile` is treated as `Dockerfile.template` and rendered on the fly (no per-service Dockerfile). Otherwise use the given Dockerfile. Then `docker build -t {image_name}:tilt`; push or `kind load`. Used by Tilt.
- **`rerp docker render-dockerfile --template PATH --service NAME -o PATH`** — Render `Dockerfile.template` to a file (for CI: render then `docker build -f path`).
...
- **`rerp build microservices [arch] [--release]`** — Build `microservices/` workspace (accounting). `arch`: amd64, arm64, arm7 (default amd64). Replaces `scripts/build-microservice.sh workspace`. **CI opts in jemalloc for amd64/arm64**; arm7 builds without jemalloc to avoid musl `__ffsdi2` link errors.
