...
def run_microservices_fmt(project_root: Path) -> int:
    """If microservices/ has changed vs HEAD, run cargo fmt in components and entities.
    ...
    # Run cargo fmt in components and entities (just fmt-rust)
    ...
    for d in ("components/", "entities/"):
        r = _run(["git", "diff", "--exit-code", "--", d], cwd=project_root)
        if r.returncode != 0:
            print(
                f"cargo fmt changed {d}. Please run: git add components entities && git commit",
                file=sys.stderr,
            )
            return 1
    return 0
