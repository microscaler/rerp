...
`entities/src/accounting/foundation/`. `0002` contains database behavior that
Lifeguard entity metadata does not yet express: tenant-consistent composite
...
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d rerp_phase1_test \
  -f sql/rls/v1/install.sql
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d rerp_phase1_test \
  -f migrations/accounting/foundation/0001_generated_entities.sql
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d rerp_phase1_test \
  -f migrations/accounting/foundation/0002_controls_and_rls.sql
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d rerp_phase1_test \
  -f tests/sql/accounting_foundation_acceptance.sql
