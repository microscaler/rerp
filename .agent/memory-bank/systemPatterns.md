...
**Naming:** `PACKAGE_NAMES` (Cargo -p, brrtrouter info.title) vs `BINARY_NAMES` (Docker/Helm/build_artifacts); can differ (e.g. invoice→invoice_management pkg, invoice binary). RERP must keep suite-nested paths (`openapi/{suite}/{service}`, `microservices/{suite}/{service}`) but should follow Hauliage's proven crate split: impl `rerp_{suite}_{service}`, gen `rerp_{suite}_{service}_gen`. The `rerp` wrapper reads impl `Cargo.toml` for build targets so it does not accidentally build gen crates during migration.
