...
# Connects to shared-kind-cluster (kind-kind) managed by microscaler/shared-kind-cluster.
#
# Layout:
...
bootstrap_rerp_role_and_db() {
  ...
  kubectl exec -i -n "${NS}" "deployment/${DEPLOY}" -c postgres -- \
    sh -c 'env PGPASSWORD="$POSTGRES_PASSWORD" psql -h 127.0.0.1 -p 5432 -U postgres -d postgres -v ON_ERROR_STOP=1' <<EOF
...
grant_rerp_dml() {
  ...
  kubectl exec -i -n "${NS}" "deployment/${DEPLOY}" -c postgres -- \
    sh -c 'env PGPASSWORD="$POSTGRES_PASSWORD" psql -h 127.0.0.1 -p 5432 -U postgres -d rerp -v ON_ERROR_STOP=1' <<'EOF'
