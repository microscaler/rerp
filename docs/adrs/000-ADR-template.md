...
A requirement for an ADR template in order to document the architecture decisions made in the project. The template should be easy to use and should help in documenting the decisions made, the reasons behind them, and their implications that is now required 
for the project. The template should be flexible enough to accommodate different types of decisions and should be able to capture the necessary information without being too cumbersome, and the permisability to have omissions of headings.
